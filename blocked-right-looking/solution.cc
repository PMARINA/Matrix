 ...
 */
void solveTypeOne(double *dstL, double *dstU, double *srcM,
                  const int blockSize) {
  int index;
  double *&u = dstU;
  double *&m = srcM;
 ...
 * @param blockSize The width or height of a square block
 */
void solveTypeTwo(double *dstL, const double *srcU, double *srcM,
                  const int &blockSize) {
  int index;
  ...
  for (int o = 0; o < blockSize; o++) {
    ...
    for (int i = 0; i < blockSize; i++) {
      for (int j = o + 1; j < blockSize; j++) {
        srcM[i * blockSize + j] -=
            dstL[i * blockSize + o] * srcU[o * blockSize + j];
      }
    }
  }
}
 ...
 * @param blockSize The width or height of a square block
 */
void solveTypeThree(double *dstU, double *srcL, double *srcM,
                    const int &blockSize) {
  int index;
  double *&u = dstU;
  double *&m = srcM;
 ...
 *
 * @param srcL The destination L matrix block, corresponding with left-most.
 * read-only
 * @param srcU The destination U matrix block, corresponding with top-most.
 * read-only
 * @param dstM The source data from M matrix. Read & Write - schur's complement
 * @param blockSize The width or height of a square block
 */
void solveTypeFour(double *&srcL, double *&srcU, double *dstM,
                   const int blockSize) {
  int index;
  double *&u = srcU;
  double *&m = dstM;
...
double ***lu_decompose(double **blockedMatrix, const int &numBlocks,
...
double **getResultDense(double ***resultsBlocked, int numBlocks,
                        int blockSize) {
  double **results = (double **)malloc(sizeof(double *) * 2);
