...
{
    double *converted = (double *)malloc(sizeof(double) * blockSize * numBlocks * blockSize * numBlocks);
    for (int i = 0; i < numBlocks; i++)
    {
        for (int j = 0; j < numBlocks; j++)
        {
            for (int i_in = 0; i_in < blockSize; i_in++)
            {
                ...
                {
                    const int outer_index = (blockSize * i + i_in) * numBlocks * blockSize + blockSize * j + j_in;
                    converted[outer_index] = blocked[i * numBlocks + j][i_in * blockSize + j_in];
                }
            }
        }
    }
